...
const fetchOrcrData = async (requiredFilters: Record<string, string | number>) => {
  const res = await fetch("https://" + process.env.NEXT_PUBLIC_CLOUDFRONT_URL + "/" + requiredFilters.year + "-" + requiredFilters.round + "-" +
    "NEET_PG" + "-NEET_PG.json");
  if (!res.ok) {
...
export default function NeetPg() {
  ...
  const [requiredFilters, setRequiredFilters] = useState<Record<string, string | number>>({
    year: mostRecentNeetPgOrcr.year,
    round: mostRecentNeetPgOrcr.round,
  });

  const roundByYears: Record<number, number[]> = neetPgRoundByYearsGlobal;
  ...
  const [view, setView] = useState<{ name: string; key: keyof Orcr; show: boolean }[]>([
    ...
    { name: "Gender", key: "gender", show: false },
  ...
  const { data: fetchedOrcrData, isLoading, error } = useQuery({
    queryKey: ['neetPgOrcrData', requiredFilters],
